...
def optimize_nerf(
    ...
    print(f"View dependency flag = {args.view_dep_text}")
    embeddings = prepare_embeddings(sds, prompt, neg_prompt, view_dependent=bool(args.view_dep_text))
    ...
    for epoch in range(max_epoch):
        ...
        for data in train_loader:
            ...
            # interpolate to 512 size
            pred_rgb = F.interpolate(pred_rgb, size=(512, 512), mode='bicubic', align_corners=True)
            pred_depth = F.interpolate(pred_depth, size=(512, 512), mode='bicubic', align_corners=True)

            # Compuate the loss
            # interpolate text_z
            azimuth = data["azimuth"]  # [-180, 180]
            azimuth = torch.abs(azimuth)
            ...
            if not args.view_dep_text:
                ...
                ### YOUR CODE HERE ###
                if azimuth < 90:
                    # interpolate between embeddings["front"] and embeddings["side"]
                    t = azimuth / 90  # normalize to [0, 1]
                    text_cond = (1 - t) * embeddings["front"] + t * embeddings["side"]
                else:
                    # interpolate between embeddings["side"] and embeddings["back"]
                    t = (azimuth - 90) / 90  # normalize to [0, 1]
                    text_cond = (1 - t) * embeddings["side"] + t * embeddings["back"]
            ...
            if global_step % 100 == 0:
                ...
                # also log depth images
                pred_depth = pred_depth.detach().cpu().permute(0, 2, 3, 1).numpy()
                pred_depth = (pred_depth - pred_depth.min()) / (
                    pred_depth.max() - pred_depth.min() + 1e-6
                )
                pred_depth = (pred_depth * 255).round()
                depth = Image.fromarray(pred_depth[0].squeeze(-1).astype("uint8"))
                output_path = (
                    f"{sds.output_dir}/images/depth_epoch_{epoch}_iter_{global_step}.png"
                )
        ...
        if epoch % log_interval == 0 or epoch == max_epoch - 1:
            ...
            with torch.no_grad():
                for i, data in enumerate(test_loader):
                    ...
                    # convert pred to shape (B,3,H,W) for interpolation
                    preds = outputs["image"].reshape(B, H, W, 3).permute(0,3,1,2)
                    preds_depth = outputs["depth"].reshape(B, 1, H, W)
                    
                    preds = F.interpolate(preds, size=(512, 512), mode='bilinear', align_corners=True)
                    preds_depth = F.interpolate(preds_depth, size=(512, 512), mode='bilinear', align_corners=True)
                    
                    # convert from (B,3,H,W) -> (B,H,W,3)
                    preds = preds.permute(0,2,3,1)
                    preds_depth = preds_depth.permute(0,2,3,1)
                    ...
                    # unsqueeze from (H,W,1) -> (H,W)
                    pred_depth_unsqueezed = pred_depth.squeeze(-1)

                    all_preds.append(pred)
                    # all_preds_depth.append(pred_depth)
                    all_preds_depth.append(pred_depth_unsqueezed)
            ...
            #     macro_block_size=1,
            # )
            imageio.mimsave(os.path.join(sds.output_dir, "videos", f"rgb_ep_{epoch}.gif"), all_preds, loop=1000, duration = 0.05)
            imageio.mimsave(os.path.join(sds.output_dir, "videos", f"depth_ep_{epoch}.gif"), all_preds_depth, loop=1000, duration = 0.05)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--prompt", type=str, default="an ice cream sundae")
    ...
    ### shading options
    parser.add_argument('--latent_iter_ratio', type=float, default=0.3, help="training iters that only use albedo shading")
