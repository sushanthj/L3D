...
def optimize_mesh_texture(
    ...
    # generate a list of query cameras after randomly sampling the camera pose
    query_cameras = []
    query_lights = []
    for i in range(10):
        # define random azimuth and elevation to get pose of the camera
        azimuth = np.random.rand() * 360
        elevation = np.random.rand() * 180 - 90
        R, T = look_at_view_transform(2.7, azimuth, elevation)
        query_cameras.append(FoVPerspectiveCameras(device=device, R=R, T=T))
        # define a light position
        query_lights.append(pytorch3d.renderer.PointLights(location=[[0, 0, -3]], device=device))
    ...
    for i in tqdm(range(total_iter)):
        ...
        # randomly sample an index of the query camera
        idx = np.random.randint(len(query_cameras))
        img = renderer(mesh, cameras=query_cameras[idx], lights=query_lights[idx]) # (N, H, W, 4)
        # Convert to (N, 3, H, W)
        img = img[..., :3].permute(0, 3, 1, 2)
        # Encode the rendered image to latents
        latents = sds.encode_imgs(img)
        # Compute the loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, default="a redback spider")
