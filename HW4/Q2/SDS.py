...
class SDS:
    ...
    def sds_loss(
        ...
        with torch.no_grad():
            ...
            predicted_noise = self.unet.forward(sample=x_t, timestep=t, encoder_hidden_states=text_embeddings).sample

            if text_embeddings_uncond is not None and guidance_scale != 1:
                ### YOUR CODE HERE ###
                predicted_noise_uncond = self.unet.forward(sample=x_t, timestep=t,
            ...
            w = 1 - self.alphas[t]
            ### YOUR CODE HERE ###
            grad = grad_scale * w[:, None, None, None] * residual
