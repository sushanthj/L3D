...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        rotation_mat = transforms.quaternion_to_matrix(quats)  # (N, 3, 3)

        if self.is_isotropic:

            ### YOUR CODE HERE ###
            # cov_3D = (N, 3, 3)
            # scales = (N, 1) -> convert to (N,3) -> diagonalize to (N, 3, 3)
            scales_diag = torch.diag_embed(scales.repeat(1, 3))
            cov_3D = rotation_mat @ scales_diag @ scales_diag.transpose(1, 2) @ rotation_mat.transpose(1, 2)
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            # cov_3D =  (N, 3, 3)
            # scales = (N, 3)
            # cov_3D = R @ S @ S.T @ R^T = (3x3) @ (3x1) @ (1x3) @ (3x3) = (3x3)

            # Convert scales to (N, 3, 3) by diagonalizing the scales
            scales_diag = torch.diag_embed(scales)  # (N, 3, 3)
            cov_3D = rotation_mat @ scales_diag @ scales_diag.transpose(1, 2) @ rotation_mat.transpose(1, 2)
    ...
    def compute_cov_2D(
        ...
        # of this function?
        # W = world to camera rotation using the PerspectiveCameras object
        W = camera.get_world_to_view_transform().get_matrix() # (N, 4, 4)
        W = W[:, :3, :3].repeat(J.shape[0],1,1)  # (N, 3, 3)
        ...
        # Σ′ = 𝐽 𝑊 Σ 𝑊.𝑇 𝐽.𝑇
        # (5,2,3) @ (5,3,3) @ (5,3,3) @ (5,3,3) @ (5,3,2) = (5,2,2)
    ...
    def compute_means_2D(means_3D: torch.Tensor, camera: PerspectiveCameras):
        ...
        # Use the PerspectiveCamera of Pytorch3D to transform the 3D means to 2D means
        means_2D = camera.transform_points_screen(means_3D)
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        # NOTE: points_2D.shape = (1, H*W, 2); H*W = number of pixels in the image, 2 = x, y coordinates of each pixel
        #          (points_2D - means_2D.shape) = (N, H*W, 2); N = number of 2D Gaussians,
        #          H*W = number of pixels,
        #          2 = x, y distances of each pixel from the mean of each gaussian
        """
        Method 1: Some memory leak issue, but nice intuition
        power = (points_2D - means_2D) @ cov_2D_inverse @ (points_2D - means_2D).transpose(1, 2)
        power = -0.5 * torch.diagonal(power, dim1=1, dim2=2)

        Why take diagonal elements for power?
        Ans. The equation (points_2D - means_2D) @ cov_2D_inverse @ (points_2D - means_2D).transpose(1, 2) is essentially Mahalanobis distance

        However, we’re interested in the diagonal of this tensor,
        because the diagonal elements represent the Mahalanobis distance for each point
        and each Gaussian. That’s why we use torch.diagonal(power, dim1=1, dim2=2).
        This gives us a tensor of shape (N, H*W), where each row represents the
        Mahalanobis distance for each point in points_2D to the corresponding Gaussian.
        """

        dists = (points_2D - means_2D).unsqueeze(-1) # (N, H*W, 2, 1)
        cov_2D_inverse = cov_2D_inverse.unsqueeze(1) # (N, 2, 2) -> (N, 1, 2, 2)
        power = -0.5 * dists.transpose(2, 3) @ cov_2D_inverse @ dists  # (N, H*W, 1, 1)
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        # get distance from image plane to each gaussian using gaussian_means
        z_vals = camera.get_world_to_view_transform().transform_points(gaussian_means)[:, 2]
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        # clip all z_vals which are less than 0
        idxs_filtered = idxs[z_vals[idxs] >= 0] # (N,)
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        # alphas = opacity @ exp(power) = (N,) @ (N, H*W) = (N, H*W)
    ...
    def compute_transmittance(
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        # transmittance = (1 - alpha_1) * (1 - alpha_2) * ... * (1 - alpha_N) for each pixel
        # transmittance.shape = (N, H, W)
        transmittance = torch.cumprod(one_minus_alphas, dim=0)  # (N+1, H, W)
        # get rid of the last element as it is not needed
    ...
    def splat(
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation
        # color at each pixel = sum(color_of_gaussian * alpha_of_gaussian * transmittance_of_gaussian)
        # image = (N, H, W, 3) @ (N, H, W, 1) @ (N, H, W, 1) = (H, W, 3)
