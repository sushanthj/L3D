...
from utils import create_dir, viz_cls_less_pts
...
def create_parser():
    ...
    parser.add_argument('--output_dir', type=str, default='./output_100pts_cls')
...
if __name__ == '__main__':
    ...
    for batch in test_dataloader:
        ...
        labels = labels.to(args.device).to(torch.long) # Nx1
        ...
        for i in randints:
            ...
            if labels[i] == pred_labels[i]:
                # Visualize Segmentation Result (Pred VS Ground Truth)
                viz_cls_less_pts(point_clouds[i], "{}/true_{}_{}.gif".format(args.output_dir, pred_labels[i], i), args.device)
            else:
                # Visualize Segmentation Result (Pred VS Ground Truth)
                viz_cls_less_pts(point_clouds[i], "{}/false_pred_{}_gt_{}_{}.gif".format(args.output_dir, pred_labels[i], labels[i], i), args.device)
